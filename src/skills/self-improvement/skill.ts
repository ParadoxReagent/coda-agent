...
export class SelfImprovementSkill implements Skill {
  ...
  constructor(config?: {
    ...
    this.githubOwner = config?.executor_github_owner ?? "";
    this.githubRepo = config?.executor_github_repo ?? "";
  }
  ...
  private static readonly PROMPT_SECTION_FILES: Record<string, string> = {
    identity: "src/core/prompts/soul.md",
    guidelines: "src/core/prompts/guidelines.md",
    security: "src/core/prompts/security.md",
    memory_instructions: "src/core/prompts/memory.md",
  };

  private async createPromptPR(
    proposal: { id: string; title: string; description: string | null },
    newContent: string,
    sectionName: string,
    version: number
  ): Promise<string | undefined> {
    if (!this.skillRegistry || !this.githubOwner || !this.githubRepo) return undefined;

    const filePath = SelfImprovementSkill.PROMPT_SECTION_FILES[sectionName];
    if (!filePath) {
      this.logger?.warn({ sectionName }, "No file mapping for prompt section, skipping PR creation");
      return undefined;
    }

    const timestamp = Date.now();
    const branchName = `agent/prompt-evolution/${sectionName}-v${version}-${timestamp}`;
    ...
    try {
      ...
      await this.skillRegistry.executeToolCall("create_or_update_file", {
        ...
        message: `[Agent] Prompt evolution: ${sectionName} v${version}\n\n${proposal.title}`,
        branch: branchName,
      });

      const body = [
        `## Prompt Evolution: \`${sectionName}\` → v${version}`,
        "",
        ...
        "",
        "---",
        "*Auto-generated by the weekly reflection cycle. The DB has already been updated — this PR tracks the change for redeployment.*",
      ].join("\n");

      const result = await this.skillRegistry.executeToolCall("create_pull_request", {
        owner: this.githubOwner,
        repo: this.githubRepo,
        title: `[Agent] Prompt: ${sectionName} v${version} — ${proposal.title}`.slice(0, 70),
        body,
        head: branchName,
        base: "main",
        draft: false,
      ...
      const prUrl = typeof result === "string"
        ? (JSON.parse(result) as { url?: string; html_url?: string })?.html_url ?? result
        : undefined;

      this.logger?.info({ sectionName, version, branchName, prUrl }, "Prompt evolution PR created");
      return prUrl;
    } catch (err) {
      this.logger?.warn({ error: err, sectionName, branchName }, "Failed to create prompt evolution PR (DB change still applied)");
      return undefined;
    }
  }
  ...
  private async decideProposal(input: Record<string, unknown>): Promise<string> {
    ...
    if (decision === "approved" && proposal.category === "prompt" && this.promptEvolutionEnabled && this.promptManager) {
      try {
        if (proposal.proposedDiff && proposal.targetSection) {
          ...
          // Create a PR so the change is tracked for redeployment
          prUrl = await this.createPromptPR(proposal, proposal.proposedDiff, proposal.targetSection, version);
        }
      } catch (err) {
    ...
    return JSON.stringify({
      ...
      message: decision === "approved"
        ? `Proposal approved${finalStatus === "applied" ? " and applied" : ""}${prUrl ? `. PR: ${prUrl}` : ""}.`
        : "Proposal rejected.",
    });
  }
